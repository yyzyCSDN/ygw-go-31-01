...
// BindIfAbsent pins a key to a node only when the key has no binding yet. It
// returns true when the binding was created. Retry paths use it so a single
// fallback success can never rewrite an existing routing decision.
func (s *Store) BindIfAbsent(key, nodeID string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) InvalidateNode(nodeID string) int {
	// BUG(09b): node-scoped invalidation is a no-op, so bindings pointing at
	// an evicted node are never removed.
	return 0
}
