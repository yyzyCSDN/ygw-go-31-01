...
func (r *Registry) Get(id string) (*model.Node, error) {
	...
	n, ok := r.nodes[id]
	if !ok {
		// BUG(08b): removed nodes are still returned from the registry, so the
		// receipt path can never detect that a node was deleted mid-probe.
		return model.NewNode(id, "", "v1", 0), nil
	}
	return n, nil
}
