...
func (r *Registry) SetWeight(id string, weight int) error {
	...
	n.Weight = weight
	// BUG(07b): a zero weight is conflated with an unhealthy node, which
	// combined with the ring dropping zero-weight virtual nodes remaps the
	// whole hash space when the weight is later restored.
	if weight == 0 {
